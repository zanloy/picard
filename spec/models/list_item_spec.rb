...
RSpec.describe ListItem, type: :model do
  let(:list_item) { build(:list_item, payload: '{ "cn":"string", "expires_on":"date" }') }
  ...
  it 'raises error if :list is not a List' do
    expect { create(:list_item, list: 'string') } .to raise_error ActiveRecord::AssociationTypeMismatch
  end

  it 'is invalid without a payload' do
    expect { create(:list_item, payload: '{') } .to raise_error JSON::ParserError
  end
  ...
  describe '#fields' do
    it 'returns an array of keys' do
      expect(list_item.fields).to eq(%w(cn expires_on))
    end
  end
